function Bear() {
    ...
    //Move the bear by xDir steps in the horiztonal axis and vertical axis
...
function restart() {
    ...
    duration.innerHTML = 0;// sets the duration to 0
    updateTimer = clearTimeout();// clear the timer
    removeBees();// clears the bees array by removing the bees
    start();//by using the onclick to call start 
}
...
function setSpeed() {
    ...
    //Update the speed to the value we get from the input field
}
class Bee {
...
function addBee() {
    let nbBees = document.getElementById("nbBees").value; //Gets the number of bees specified by the user
    nbBees = Number(nbBees); 
    //Convert the content of the input to a number
    nbBees++;
    var bee = new Bee(nbBees); //Creates a bee
    bee.display(); //Displays the bee 
    bees.push(bee); 
    //Adding the bee to the bees array
}

function removeBees() {
    beesArray = document.getElementsByClassName("bee");
    beesArray.forEach(e => e.remove()); //removing the bee
}
...
function updateBees() {
    ...
    moveBees();
    //use the input field periodTimer instead of the fixed period 
    let period = document.getElementById("periodTimer").value;

    let score = hits.innerHTML;
    if (Number(score) < 1000) {
        updateTimer = setTimeout('updateBees()', period);
        //Update the bees movement after the specified interval
    } else {
        score = "Game Over"
        hits.innerHTML = score;
